...
/**
 * 연합 뉴스 RSS 피드를 통해 뉴스 기사를 수집하는 컴포넌트입니다.
 * <p>
...
public class YonhapRssFetcher implements ArticleFetcher {
    ...
    public List<ArticleSaveRequest> fetch(String keyword) {
        log.info("연합 뉴스 RSS 뉴스 수집 시작 = Keyword : {}",  keyword);
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                log.error("연합 뉴스 RSS 피드 응답 실패 statusCode = {}", response.statusCode());
                return List.of();
            }
            ...
            log.info("연합 뉴스 RSS 뉴스 수집 완료. 총 {}건", articles.size());
            return articles;

        } catch (Exception e) {
            log.error("연합 뉴스 RSS 뉴스 수집 중 예외 발생", e);
            return List.of();
        }
